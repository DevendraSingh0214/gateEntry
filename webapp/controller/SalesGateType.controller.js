sap.ui.define([
    ...
    function (Controller, UIComponent, MessageToast, MessageBox) {
        ...
        return Controller.extend("zgateentry.controller.SalesGateType", {
            onInit: function () {
                this.getView().setModel(new sap.ui.model.json.JSONModel, "oTableDataModel");
                UIComponent.getRouterFor(this).getRoute('SalesGateType').attachPatternMatched(this.ScreenRefrash, this);
                UIComponent.getRouterFor(this).getRoute('SalesGateType').attachPatternMatched(this.CallGateEntryData, this);
            },

            ChallanQty_Function: function () {
                const input1 = this.getView().byId("ChallanQty");
                const input2 = this.getView().byId("ContainerNo");
                input2.focus();
            },
            ...
            ScreenRefrash: function () {
                ...
                this.getView().byId("DeliveryNo").setValue()
            ...
            disableFieldsEditable: function () {
                ...
                if (GateEntryAction === "Display") {
                    ...
                    this.getView().byId("GateOutDate").setEditable(false);
                    this.getView().byId("GateOutTime").setEditable(false);
                    this.getView().byId("DeliveryNo").setEditable(false);
                    ...
                    this.getView().byId("DeliveryNo").setEditable(true);
            ...
            GetTableData: function () {
                ...
                oBusy.open();
                var delivaryNumber = this.getView().byId("DeliveryNo").getValue();
                if (delivaryNumber == "" || delivaryNumber == "NaN" || delivaryNumber == undefined || delivaryNumber == 0) {
                    MessageBox.error("Please Enter Delivary Number First")
                } else {
                    var oFilter = new sap.ui.model.Filter("DeliveryDocument", "EQ", delivaryNumber);
                    var oFilter_Gate_Item = new sap.ui.model.Filter("Delievery", "EQ", delivaryNumber);
                    var oModel = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/Y1416_GATE");
                    var oTableModel = this.getView().getModel("oTableDataModel");
                    var aTableArr = oTableModel.getProperty("/aTableData");
                    var delivaryNumber1 = this.getView().byId("DeliveryNo")
                    oModel.read("/gate_item", {
                        filters: [oFilter_Gate_Item],
                                    ...
                                    success: function (oresponse) {
                                        if (aTableArr.length === 0) {
                                            oresponse.results.map(function (items) {
                                                ...
                                                //     num2 = items.deliver;
                                                // }
                                                let floatValue = parseFloat(items.delievered_quantity).toFixed(2);
                                                var delievered_quantity = (floatValue).toString();
                                                aTableArr.push({
                                                    ...
                                                    "DelivaryQuantity": delievered_quantity,
            ...
            DeleteTables_SelectedRow_Button_Visible: function () {
                var GateEntryAction = this.getView().getModel("oFirstScrennDataModel").getProperty("/GateEntryAction")
                if (GateEntryAction != "Display") {
                    var oTable = this.getView().byId("SalesGateTypeTable").getSelectedIndices();
                    if (oTable.length > 0) {
            ...
            DeleteTables_SelectedRow: function (oEvent) {
                ...
                var oTable = this.getView().byId("SalesGateTypeTable");
