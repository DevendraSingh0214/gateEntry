sap.ui.define([
    ...
    function (Controller, UIComponent, MessageToast, Fragment, MessageBox, FilterOperator, Filter) {
        ...
        return Controller.extend("zgateentry.controller.NonReturnable", {
            onInit: function () {
                this.getView().setModel(new sap.ui.model.json.JSONModel, "oTableDataModel");
                UIComponent.getRouterFor(this).getRoute('NonReturnable').attachPatternMatched(this.ScreenRefrash, this);
                UIComponent.getRouterFor(this).getRoute('NonReturnable').attachPatternMatched(this.CallGateEntryData, this);
                // UIComponent.getRouterFor(this).getRoute('NonReturnable').attachPatternMatched(this.CallValueHelpData, this);
            },
            CallValueHelpData: function () {
                var oBusy = new sap.m.BusyDialog({
                    text: "Please Wait"
                });
                oBusy.open();
                var oModel = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/Y1416_GATE");
                var oModel1 = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/ZCUST_F4")
                this.getView().setModel(new sap.ui.model.json.JSONModel, "oVendorCodeDataModel");
                this.getView().getModel("oVendorCodeDataModel").setProperty("/aVendorCodeData", []);
                var oVendorCodeValueHelpModel = this.getView().getModel("oVendorCodeDataModel")
                var aVendorData = [];
                oModel.read("/SUPPLIER", {
                    ...
                    success: function (oresponse) {
                        oresponse.results.map(function (items) {
                            ...
                            aVendorData.push(obj)
                        })
                        oVendorCodeValueHelpModel.setProperty("/aVendorCodeData", aVendorData);
                    }.bind(this)
                })
                oModel1.read("/ZCust_f4", {
                    urlParameters: {
                        "$top": "5000"
                    },
                    success: function (oresponse) {
                        oresponse.results.map(function (items) {
                            var obj = {
                                "Supplier": items.Customer,
                                "SupplierFullName": items.CustomerName,
                                "VendorType": "Customer",
                            }
                            aVendorData.push(obj)
                        })
                ...
                oModel.read("/zgat", {
                    ...
                    success: function (ores) {
                        if (ores.results.length > 0) {
                            ...
                            var timeD = ores.results[0].GateOutTm.ms;
                            ...
                            this.getView().byId("GateOutTime").setValue(time)
                            ...
                            this.getView().byId("InvoiceDate").setValue(ores.results[0].Invdt)
            ...
            DeleteTables_SelectedRow: function (oEvent) {
                ...
                var oModel = new sap.ui.model.odata.v2.ODataModel("/sap/opu/odata/sap/Y1416_GATE");
                var gateEntryNumber = this.getView().byId("GateEntryNumber").getValue();
                var oTable = this.getView().byId("NonReturnableGateTypeTable");
                ...
                this._pValueHelpDialog.then(function (oValueHelpDialog) {
                    if (sKey === 'VendorCode') {
                        var oTemplate = new sap.m.StandardListItem({
                            title: "{oVendorCodeDataModel>Supplier}",
                            description: "{oVendorCodeDataModel>SupplierFullName}",
                            info: "{oVendorCodeDataModel>VendorType}",
                            type: "Active"
                        });
                        oValueHelpDialog.bindAggregation("items", {
                            path: 'oVendorCodeDataModel>/aVendorCodeData',
                            template: oTemplate
                        });
